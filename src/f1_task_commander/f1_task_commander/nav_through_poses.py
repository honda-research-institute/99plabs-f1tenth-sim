...
class NavThroughPosesNavigator(Node):
    def __init__(self):
    ...
    def config_setup(self):
        """
        Set up the configuration for the NavThroughPosesNavigator.
        ...
        try:
            ...
            self.declare_parameter('localizer', rclpy.Parameter.Type.STRING)
            self.declare_parameter('task_params_file', rclpy.Parameter.Type.STRING)
            self.declare_parameter('timeout_wait', rclpy.Parameter.Type.DOUBLE)
            # Get parameters and verify types
            self.params_file = self.get_parameter('task_params_file').value
            params_file =yaml.safe_load(open(self.params_file))
            print(params_file)
            self.params = params_file['task_commander']['ros__parameters']
            
            self.waypoints_file = self.get_parameter('waypoints_file').value
            self.timeout_wait = float(params_file['task_commander']['ros__parameters']['timeout_wait'])
            self.localizer = params_file['task_commander']['ros__parameters']['localizer']
    ...
    def execute_navigation(self):
        ...
        self.get_logger().info(f"Number of Poses: {len(goal_poses)}")
        ...
        while not self.navigator.isTaskComplete():
            ...
            if feedback and i % 5 == 0:
                print(f'Estimated time of arrival: {Duration.from_msg(feedback.estimated_time_remaining).nanoseconds / 1e9} seconds')
...
def main(args=None):
    ...
    nav_through_poses_navigator = NavThroughPosesNavigator()
    rclpy.spin(nav_through_poses_navigator)
