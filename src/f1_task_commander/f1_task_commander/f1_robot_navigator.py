...
from nav2_msgs.action import AssistedTeleop, BackUp
...
class BasicNavigator(Node):
    ...
    def __init__(self, node_name='basic_navigator', namespace='', localizer='amcl'):
        ...
        # Check if localizer is amcl or slam_toolbox, set coresponding params
        if self.localizer == 'amcl':
            ...
            self.localization_pose_sub = self.create_subscription(PoseWithCovarianceStamped,
                                                              ...
                                                              amcl_pose_qos)
        elif self.localizer == 'slam_toolbox':
            self.localization_pose_sub = self.create_subscription(PoseWithCovarianceStamped,
                                                              'pose',
                                                              self._slam_toolboxPoseCallback,
                                                             )
        else: 
            self.get_logger().error('Unknown localizer type...')
    ...
    def waitUntilNav2Active(self, navigator='bt_navigator', localizer='amcl'):
        ...
        if localizer == 'amcl' or localizer == 'slam_toolbox':
            self.info(f'Localizer {localizer} is being used...')
            self._waitForInitialPose()
        else:
            self.info(f'Localizer {localizer} is not yet supported')
    ...
    def _waitForInitialPose(self):
        while not self.initial_pose_received:
            ...
            self._setInitialPose()
            self.info('Waiting for initial pose to be received')
    ...
    def _slam_toolboxPoseCallback(self, msg):
        self.debug('Received Scan Matching pose')
        self.initial_pose_received = True
        return
